...
class DeepSpeakerTestDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        enroll_utt_id_list, test_utt_id, enroll_feat_path_list, test_feat_path, label = self.pair_indices[pair]
        enroll_num = len(enroll_utt_id_list)
        enroll_feature_mat_list = []
        for x in range(enroll_num):
            enroll_utt_id = enroll_utt_id_list[x]
            if len(enroll_utt_id_list) == 1:
                enroll_feat_path = enroll_feat_path_list
            else:
