...
class dcf_gsc_enh(BaseModel):
    def __init__(self, basic_args, multiple_args):
            ...
            if not hasattr(multiple_args, 'DCF_Ratio'):
                multiple_args.DCF_Ratio = 0.50
    ...
    def inference(self, input_audio, tdoas):
        with torch.no_grad():
            ...
            if self.multipleEnh is not None:
                ...
                multiple_enh_fft, emask = self.multipleEnh(mFFT, self.tdoas, cmvn = cmvn, fbank_extractor = fbank_extractor, DCF_Ratio = self.multiple_args.DCF_Ratio) # ( num_block, num_frame, 2, num_bin), ( num_block, num_frame, num_bin )
    ...
    def forward(self):
        ...
        if self.multipleEnh is not None:
            ...
            multiple_enh_fft, emask = self.multipleEnh(self.mFFT, self.tdoas, cmvn = cmvn, fbank_extractor = fbank_extractor, DCF_Ratio = self.multiple_args.DCF_Ratio) # ( num_block, num_frame, 2, num_bin), ( num_block, num_frame, num_bin )
