...
class DPR_DSNR(nn.Module):
    ...
    """
    def __init__(self, num_beam, num_bin, num_channel, targ_bf_weight = None, fix = True):
        super(DPR_DSNR, self).__init__()
    ...
    def forward(self, input, beam_id):
        ...
        max_bf_power  = targ_bf_power                                               # (num_block, num_frame, num_bin)
        for tdoa in torch.arange(self.num_beam):
            for n in torch.arange(num_block):
                ...
                max_bf_power[n, :, :] = torch.where( max_bf_power[n, :, :] > bf_power, max_bf_power[n, :, :], bf_power )

        dpr  = targ_bf_power / sum_bf_power # (num_block, num_frame, num_bin)
        dsnr = targ_bf_power / max_bf_power # (num_block, num_frame, num_bin)

        return dpr, dsnr, targ_bf_out.squeeze(4) # (num_block, num_frame, num_bin), (num_block, num_frame, num_bin), (num_block, num_frame, 2, num_bin)
