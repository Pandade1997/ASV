...
from .attractor import fa_attention, SimAttractor#, FFAttractor, RNNAttractor
...
class FCFALayer(nn.Module):
    ...
    def get_trainable_params(self, print_info = False):
        ...
        if self.attractor is not None:
            for name, param in self.attractor.named_parameters():
                frequency_align_param_names.append(name)
                if 'bias' in name:
                    biases += [param]
                else:
    ...
    def forward(self, input, guid_info = None, num_block = 1, numerical_protection = 1.0e-13, compressed_scale = 0.3):
        # input:         ( num_frame, 2, num_bin, num_beam ) * (num_beam, num_align)
        # guid_info: ( num_block, 2, num_bin, num_frame )
        ...
        if self.pooling_type is not None:
            if self.pooling_type.lower() == 'avg':
                ...
                align_out, gate_mask = self.gate(align_out, num_block = num_block, numerical_protection = numerical_protection, compressed_scale = compressed_scale)
            elif self.pooling_type.lower() == 'attention':
                value = align_out                                               # ( num_frame, 2, num_bin, num_align)
                if guid_info is not None:
                    # guid_info: ( num_block, 2, num_bin, num_frame )
                    query = guid_info[:,0,:,:] ** 2 + guid_info[:,1,:,:] ** 2   # ( num_block, num_bin, num_frame )
                    ...
                    align_out, att_v = self.attractor(query, key, value)          # ( num_frame, 2, num_bin ), ( num_frame, 2, num_align )
                    #align_out, att_v = fa_attention(query, key, value)          # ( num_frame, 2, num_bin ), ( num_frame, 2, num_align )
                else:
                    print("guid_info is None")
                    exit(1)
            else:
...
class Conv2DFALayer(nn.Module):
    ...
    def forward(self, input, guid_info = None, num_block = 1, numerical_protection = 1.0e-23, compressed_scale = 0.3):
