...
class GSC(nn.Module):
    ...
    def forward(self, input, mask, beam_id, alpha_v = 0.95, DCF_Ratio = 0.5):
        ...
        for t in torch.arange(1, num_frame):
            ...
            alpha_b     = torch.clamp(alpha_b, max = 0.999)    # ( num_block, num_bin )
            ...
            #out_r = out_r.view(num_block, 1, 1, num_bin) # ( num_block, 1, 1, num_bin)
            #out_i = out_i.view(num_block, 1, 1, num_bin) # ( num_block, 1, 1, num_bin)
