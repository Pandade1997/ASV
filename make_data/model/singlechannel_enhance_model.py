...
class multiple_single_enh(BaseModel):
    def __init__(self, basic_args, multiple_args, single_args):
            ...
            if self.basic_args.isTrain:
                ...
                self.criterionL1 = torch.nn.MSELoss() # self.criterionL1 = torch.nn.L1Loss()
    ...
    def set_input(self, input_audio, target_audio, input_lengths = None):
        """
        Args:
    ...
    def inference(self, input_audio, target_audio = None):
        ...
        with torch.no_grad():
            ...
            if target_audio is not None:
                target_audio = target_audio.to(self.device) # (num_block, 1, num_sample)
                sFFT = self.convstft(target_audio)          # (num_block, num_bin * 2, num_frame)
                sFFT = torch.cat([torch.unsqueeze(sFFT[:, :self.num_bin, :], 1), torch.unsqueeze(sFFT[:, self.num_bin:, :], 1)], dim = 1)
            else:
                sFFT = None
            ...
            if self.multipleEnh is not None:
                multiple_enh_fft = self.multipleEnh(mFFT, target_speech = sFFT, num_block = num_block) # ( num_block * num_frame, 2, num_bin)
            ...
            if self.singleEnh is not None:
                single_input_fft = mFFT[:, :, :, 0]
                single_input_fft = single_input_fft.view(num_block, num_frame, 2, self.num_bin) # ( num_block, num_frame, 2, num_bin)
                single_input_fft = single_input_fft.permute([0, 2, 3, 1]).contiguous()                    # ( num_block, 2, num_bin, num_frame )
                single_enh_fft   = self.singleEnh(single_input_fft)                                  # ( num_block, 2, num_bin, num_frame )
    ...
    def forward(self):
        ...
        if self.multipleEnh is not None:
            self.multiple_enh_fft = self.multipleEnh(self.mFFT, target_speech = self.sFFT, num_block = self.num_block)                                                 # ( num_block * num_frame, 2, num_bin)
        ...
        if self.singleEnh is not None:
            single_input_fft = self.mFFT[:, :, :, 0]
            single_input_fft = single_input_fft.view(self.num_block, self.num_frame, 2, self.num_bin) # ( num_block, num_frame, 2, num_bin)
            single_input_fft = single_input_fft.permute([0, 2, 3, 1]).contiguous()                    # ( num_block, 2, num_bin, num_frame )
            self.single_enh_fft   = self.singleEnh(single_input_fft)                                  # ( num_block, 2, num_bin, num_frame )
        else:
        ...
        if self.compressed_scale != 1.0:
            #scale = torch.pow(spe_spect + self.numerical_protection, 0.5 * (self.compressed_scale - 1.0))
            if spe_spect is not None:
                scale = spe_spect ** (0.5 * self.compressed_scale)
                scale = torch.cat((scale, scale), dim = 1)
            else:
                scale = None
            if self.sFFT is not None and scale is not None:
                self.sFFT             = scale * self.sFFT
            if self.multiple_enh_fft is not None and scale is not None:
                self.multiple_enh_fft = scale * self.multiple_enh_fft                     # ( num_block, 2, num_bin, num_frame )
            if self.single_enh_fft is not None and scale is not None:
                self.single_enh_fft   = scale * self.single_enh_fft
    ...
    def backward_E(self, perform_backward = True):
        ...
        if multiple_fft_weight > 0.0 and self.multiple_enh_fft is not None:
            self.loss_multipleFFT = self.criterionL1(self.multiple_enh_fft, self.sFFT) * self.num_bin * 2
            if losses is None:
        ...
        if single_fft_weight > 0.0 and self.single_enh_fft is not None:
            self.loss_singleFFT   = self.criterionL1(self.single_enh_fft, self.sFFT) * self.num_bin * 2
            if losses is None:
