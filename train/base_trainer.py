    ...
    for i, (data) in enumerate(valid_enum, start=0):
        with torch.no_grad():                
            ...
            for enroll_feat in enroll_feat_list:
                if enroll_feat is not None:
                    inputs = enroll_feat.to(opt.device)
                    enroll_output = extract_feature(opt, inputs, model, segment_length, segment_shift)
                    enroll_output = enroll_output / torch.sqrt(torch.sum(enroll_output**2, dim=-1, keepdim=True) + 1e-6).unsqueeze(0)
            ...
            test_output = test_output / torch.sqrt(torch.sum(test_output**2, dim=-1, keepdim=True) + 1e-6).unsqueeze(0)
...
def union_evaluate(opt, model_1, model_2, val_loader, logging):
    if isinstance(model_1, DistributedDataParallel):
        model_1 = model_1.module
    if isinstance(model_2, DistributedDataParallel):
    ...
    for i, (data) in enumerate(valid_enum, start=0):
        with torch.no_grad():                
            ...
            for enroll_feat in enroll_feat_list:
                inputs = enroll_feat.to(opt.device)
                enroll_output_1 = extract_feature(opt, inputs, model_1, segment_length, segment_shift)
                enroll_output_2 = extract_feature(opt, inputs, model_2, segment_length, segment_shift)
                enroll_output = torch.cat((enroll_output_1.unsqueeze(0), enroll_output_2.unsqueeze(0)), dim=1)
                enroll_output = enroll_output / torch.sqrt(torch.sum(enroll_output**2, dim=-1, keepdim=True) + 1e-6)
                if enroll_output_all is None:
                    enroll_output_all = enroll_output
                else:
                    enroll_output_all = torch.cat((enroll_output_all, enroll_output), dim=0)  
            ...
            inputs = test_feat.to(opt.device)
            test_output_1 = extract_feature(opt, inputs, model_1, segment_length, segment_shift)
            test_output_2 = extract_feature(opt, inputs, model_2, segment_length, segment_shift)
            test_output = torch.cat((test_output_1.unsqueeze(0), test_output_2.unsqueeze(0)), dim=1)
            test_output = test_output / torch.sqrt(torch.sum(test_output**2, dim=-1, keepdim=True) + 1e-6)
            test_output = test_output.detach().cpu().numpy()     
