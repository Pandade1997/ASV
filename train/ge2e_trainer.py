...
def train(opt, logging):
    ...
    model = model_select(opt, seq_training=False)
    ...
    for epoch in range(1, opt.total_epoch + 1):
        while True:
            ...
            for i, (data) in enumerate(train_loader, start=0):
                ...
                outputs, attn, w, b = model(feature_input) 
                
                outputs = save_model.normalize(outputs)  
                sim_matrix_out = save_model.similarity(outputs, w, b)  
                ...
                if total_iters % opt.print_freq == 0:
                    ...
                    if opt.main_proc:
                        ...
                        if isinstance(lr, list):
                            lr = max(lr)
                        logging.info(
                            '==> Train set steps {} lr: {:.6f}, loss: {:.4f} [ embedding: {:.4f}, penalty_loss {:.4f}]'.format(
                                total_iters, lr, losses.avg, embedding_losses.avg, penalty_losses.avg))
